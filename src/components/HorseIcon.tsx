/**
 * Horse head SVG icon — used for ratings (filled/dim like stars)
 * and as inline decoration next to score text.
 *
 * Uses Font Awesome horse-head path (CC BY 4.0).
...
interface HorseIconProps {
  /** Filled = selected, dim = unselected */
  filled?: boolean;
  /** Pixel size */
  size?: number;
  className?: string;
...
export default function HorseIcon({
  ...
  return (
    <svg
      ...
      aria-hidden="true"
    >
      <path d={HORSE_PATH} fill="currentColor" opacity={filled ? 1 : 0.15} />
    </svg>
  );
}
