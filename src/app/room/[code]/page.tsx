  ...
  return (
    <div className="min-h-dvh flex flex-col max-w-2xl mx-auto">
      ...
      <header
        ...
        style={{
          background:
            "linear-gradient(to bottom, rgba(10,10,15,0.95) 80%, transparent)",
          backdropFilter: "blur(12px)",
        }}
      >
        <div className="flex items-center justify-between mb-3">
          <div>
            <h1 className="text-xl font-bold gradient-text">Ketamovies</h1>
            <button
              onClick={copyCode}
      ...
      <main className="flex-1 px-4 pb-8">
        {activeTab === "watchlist" && session && (
          <WatchlistTab roomCode={code} username={session.username} />
        )}
        {activeTab === "watched" && session && (
          <WatchedTab roomCode={code} username={session.username} />
        )}
        {activeTab === "stats" && (
          <StatsTab roomCode={code} />
        )}
      </main>
    </div>
  );
